...
<pre>01 9-22 * * * /Users/userdir/backup_documents
	...
	p. Now this might seem like a lot to copy up, but that's the beauty of rsync. It only does incremental copying, so it will only copy files that have changed since the last time the script ran. Of course, if you work with video files, this might be overkill, because of the time it takes to upload the files, so you can adjust to your own needs.




	p. rsync can also do standard weekly incremental backups, which is something I have yet to experiment with.




	p. Like I said, this was mainly for my own records, but hopefully it will come in handy for someone.




	p. *<span class="caps">UPDATE</span>*:
